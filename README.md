...
```sh
cd /Users/olli/.openclaw/workspace/projects/caschy-adventure
npm start
```

Dann **http://127.0.0.1:8080** im Browser öffnen. Beenden: `Ctrl+C` im Serverterminal. Der Projektpfad ist ein Symlink auf die externe SSD; Quellcode, Git-Verzeichnis und Testartefakte liegen vollständig in diesem Projekt.
...
- Der Browsertest startet und beendet seinen eigenen lokalen Server. Er liest das vorhandene Playwright-Modul über `/Users/olli/.openclaw/workspace/projects/lantern-depths/node_modules/playwright/index.mjs`. **Kein `npm install` in lantern-depths**, keine Dateien dort geändert. Das Testprofil und temporäre Testdateien liegen im eigenen Projekt und sind von Git ausgeschlossen.
...
- Die Testskripte verwenden bewusst den ausdrücklich freigegebenen absoluten Playwright-Pfad dieses Rechners. Das **Spiel** bleibt unabhängig davon auf jedem statischen HTTP-Server nutzbar.

Nur lokale Git-Historie. Kein Remote, keine Veröffentlichung und kein Deployment.
