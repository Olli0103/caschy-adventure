...
function render(tone='normal'){
 ...
 if(!state.inventory.length)$('inventory').append(el('p',{class:'empty-inventory'},'Caschys Tasche ist noch leer. Wenigstens liegen keine Abo-Karten darin.'));
...
function renderDialogue(){
 ...
 const close=el('button',{class:'close-dialogue','data-focus':'close-dialogue'},'Zurück zum Raum');close.addEventListener('click',()=>run({type:'close'}));$('dialogue').append(choices,close);
}
...
function renderEnding(){const bad=state.ending==='bad';$('ending-kicker').textContent=bad?'NEBENENDE / DAS GEHT BESSER':'FEIERABEND / 200 OK';$('ending-title').textContent=bad?'Der Preis der Klicks':'Das Blog ist wieder da.';$('ending-text').textContent=bad?'Die neue Seite besteht fast nur noch aus Kaufbuttons, Rabattcodes und einem Abo auf das eigene Feierabendbier. Der Algorithmus ist begeistert, Caschy deutlich weniger. Zum Glück lässt sich die Überschrift sofort ändern, ohne eines der Rätsel erneut lösen zu müssen.':'André legt das Proton Pack beiseite, Felix schaltet die Lichter lokal und Olli beendet seine Diagnose. Benny findet das Bier schließlich in der richtigen Tasche. Caschy hebt sein Glas: "Kein Clickbait, kein Bullshit. Wir lesen uns." Der Kuchen wird angeschnitten, und für heute braucht niemand einen Plan B.';drawEnding($('ending-canvas'),bad);$('ending-canvas').setAttribute('aria-label',bad?'Das ganze Team am Hafen, über dem Kuchen regnet Affiliate-Konfetti.':'Caschy, André, Benny, Olli und Felix feiern am Hafen mit Kuchen und Bier.');$('ending-actions').replaceChildren();
